...
#training_features = 
training_features = training.toPandas()
training_features = training_features.to_records(index=False)
...
def classify_twitter(features):
    predicted = Classifier.Classifier.classify(features)
    probability = Classifier.Classifier.prob_classify(features).prob(predicted)
    return predicted

udf_classify_twitter = functions.udf(classify_twitter)
test = test.withColumn("predicted", udf_classify_twitter(functions.col("features")))
test.show(10)